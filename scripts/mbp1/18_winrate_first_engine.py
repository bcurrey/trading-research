...
base = (
    ...
    .with_columns([
        ...
        (pl.col("atr_14") > pl.col("atr_50")).cast(pl.Int8).alias("high_volatility"),
        (pl.col("premarket_range") > 80).cast(pl.Int8).alias("large_premarket"),
        (pl.col("prior_day_range") > 250).cast(pl.Int8).alias("large_prior_day"),
        (pl.col("prior_day_range") < 120).cast(pl.Int8).alias("small_prior_day"),
        ...
        (pl.col("update_velocity") > 500).cast(pl.Int8).alias("high_velocity"),
...
print(f"Base setups: {base.height:,}")
...
print("\nRUNNING WIN-RATE-FIRST ENGINE...\n")

def grade_model(winrate, pf, trades_per_year, avg_trade, max_dd, profitable_years_pct):
    if (
        winrate >= 0.60 and
        pf >= 1.50 and
        trades_per_year >= 25 and
        avg_trade >= 5 and
        profitable_years_pct >= 0.65
    ):
        return "A+"
    if (
        winrate >= 0.55 and
...
for r in range(0, 5):
    condition_sets = [()] if r == 0 else itertools.combinations(conditions, r)
...
print(f"\nCondition combos tested: {combo_count:,}")
print(f"Valid models: {len(results):,}")
...
ranked.write_csv(OUTPUT_DIR / "winrate_first_ranked_models.csv")
ranked.filter(pl.col("tier").is_in(["A+", "A"])).write_csv(
    OUTPUT_DIR / "winrate_first_A_models.csv"
)
ranked.write_parquet(
    OUTPUT_DIR / "winrate_first_all_models.parquet",
    compression="zstd"
)
...
print("\nFILES SAVED:")
print(OUTPUT_DIR / "winrate_first_ranked_models.csv")
print(OUTPUT_DIR / "winrate_first_A_models.csv")
print(OUTPUT_DIR / "winrate_first_all_models.parquet")