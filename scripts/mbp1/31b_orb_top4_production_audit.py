...
SCRIPT_NAME = "31b_orb_top4_production_audit"
...
df = (
    ...
    .with_columns([
        ...
        pl.col("month").cast(pl.Utf8).alias("month_str"),
    ])
...
summary = (
    ...
    .agg([
        ...
        pl.col("month_str").n_unique().alias("active_months"),
        pl.col("year_int").n_unique().alias("active_years"),
...
recent = (
    df.filter(pl.col("year_int").is_in([2024, 2025, 2026]))
    .group_by("model")
    .agg([
        pl.len().alias("trades"),
        (pl.col("pnl_points") > 0).mean().round(4).alias("winrate"),
